...
describe ".calculate_amount" do
  before do
    #  把每个测试都会用到的 @time 提取出来， 这个before 区块会在这个 describe 内的所有测试前执行
    @time = Time.new(2017,3,27,8,0,0) # 固定一个时间比Time.now更好，这样每次跑测试才能确保一样的结果
  end

  context "guest" do

    before do
      # 把每个测试都会用到的 @parking 提取出来，这个before 区块会在这个 context 内的所有测试前执行
  ...
  context "short-term" do
    ...
    before do
      ...
      @user = User.create( :email => "test@example.com", :password => "123456678")
  ...
  context "long-term" do
    ...
    before do
      # 把每个测试都会用到的 @user 和 @parking 提报出来
    ...
    it "1400 mins should be ￥16" do
      @parking.end_at = @time + 1400.minutes
      @parking.save
      expect(@parking.amount).to eq(1600)
    end

    it "1450 mins should be ￥28" do
      @parking.end_at = @time + 1450.minutes
      @parking.save
      expect(@parking.amount).to eq(2800)
    end

    it "2900 mins should be ￥44" do
      @parking.end_at = @time + 2900.minutes
      @parking.save
